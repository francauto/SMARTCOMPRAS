...
import ModalNovaSolicitacaoCliente from "../modal/ModalNovaSolicitacaoCliente";
...
import { Aprovador } from "@/types/clientes";
...
export default function SolicitacoesClientesPage() {
  ...
  const handleCreateSolicitacao = (data: any) => {
    setModalNovaOpen(false);
    handleRefreshTable();
  };

  // Exibe loading global enquanto contexto de auth carrega
  if (loading) {
    return (
      <Box
        sx={{
  ...
  return (
    ...
    >
      <div className="flex-1 overflow-hidden p-4 sm:p-6 md:p-8 flex justify-center">
        <div className="w-full max-w-[95vw] xl:max-w-[1600px] flex flex-col min-h-0">
          ...
          <Table
            ...
            onRowClick={(row) => {
              // Atualiza a URL para abrir o modal de detalhes com o id
      ...
      {/* ALTERAÇÃO: Modais movidos para o final do componente para não interferir no layout. */}
      <ModalNovaSolicitacaoCliente
        open={modalNovaOpen}
      ...
      <ModalDescricaoClientes
        ...
        onStatusChange={handleRefreshTable}
      />
    </div>
  );
}
