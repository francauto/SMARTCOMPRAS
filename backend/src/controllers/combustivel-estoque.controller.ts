...
import {
  ...
  responderRequisicao,
} from "../services/combustivel-estoque.service";
import {
...
export async function criarRequisicaoCombustivelEstoqueController(
  req: Request,
  res: Response
) {
  try {
    ...
    if (!modelo || !marca || !quantidade_litros || !id_aprovador) {
    ...
    if (!chassi && !placa) {
      return res.status(400).json({
        error: "É necessário fornecer pelo menos chassi ou placa",
      });
    }

    const dados: ICriarRequisicaoCombustivelEstoque = {
      chassi: chassi || undefined,
      placa: placa || undefined,
      modelo,
      marca,
      quantidade_litros: Number(quantidade_litros),
      tipo_combustivel: tipo_combustivel || undefined,
      id_aprovador: Number(id_aprovador),
      id_solicitante,
      id_departamento: id_departamento ? Number(id_departamento) : undefined,
    };

    const requisicaoId = await criarRequisicaoCombustivelEstoque(dados);
...
export async function responderRequisicaoController(
  ...
  try {
    ...
    const dados: IRespostaRequisicaoCombustivelEstoque = {
